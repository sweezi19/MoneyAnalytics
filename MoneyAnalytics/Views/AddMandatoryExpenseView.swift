...
struct AddMandatoryExpenseView: View {
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                Button(action: {
                    if let expenseValue = Double(expense) {
                        ...
                        let newExpense = MandatoryExpense(context: viewContext)
                        newExpense.amount = expenseValue
                        newExpense.category = selectedCategory
                        do {
                            try viewContext.save()
                            print("Mandatory expense saved: \(expenseValue), category: \(selectedCategory)")
                            self.presentationMode.wrappedValue.dismiss()
                        } catch {
                            print("Failed to save mandatory expense: \(error.localizedDescription)")
                        }
                    } else {
            ...
            .padding()
            .navigationBarTitle("Add Mandatory Expense", displayMode: .inline)
        }
    }
}
